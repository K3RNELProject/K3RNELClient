...
import org.k3rnel.client.ui.base.ComboBox;
 ...
 * 
 */
public class StorageBoxFrame extends Frame {
	private ToggleButton[] m_buttons = new ToggleButton[30];
	private int[] m_monsterNums = new int[30];
	private int m_buttonChosen = 0;
	private ComboBox m_changeBox;
	private Button m_switchMonster, m_close, m_release;
	 ...
	 */
	public StorageBoxFrame(int[] m_monsters){
		getContentPane().setX(getContentPane().getX() - 1);
		getContentPane().setY(getContentPane().getY() + 1);
		m_monsterNums = m_monsters;
		m_boxIndex = 0;
	...
	public void loadImages() {
		...
		for (int i = 0; i <= 29; i++) {
			...
			try {
				if(m_monsterNums[i] >= 0) {
					f = FileLoader.loadFile(Monster
							.getIconPathByIndex(m_monsterNums[i] + 1)); 
					m_buttons[i].setImage(new Image(f, "boxPoke" + i + " " + Monster
							.getIconPathByIndex(m_monsterNums[i] + 1), false));
				}
	...
	public void setChoice(int x) {
		...
		m_switchMonster.setEnabled(true);
	...
	public void initGUI() {
		...
		m_switchMonster = new Button();
		m_close = new Button();
		m_changeBox = new ComboBox();
		m_release = new Button();

		m_switchMonster.setText("Switch");
		m_switchMonster.pack();
		m_switchMonster.setLocation(5, 192);
		m_switchMonster.setEnabled(false);
		m_switchMonster.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				setVisible(false);
		...
		m_changeBox.setLocation(m_switchMonster.getX() + m_switchMonster.getWidth(),
				197);
			...
			try {
				if (GameClient.getInstance().getOurPlayer().getMonsters()[i] != null) {
					m_level[i].setText("Lv: "
							+ String.valueOf(GameClient.getInstance().getOurPlayer().getMonsters()[i].getLevel()));
					m_level[i].pack();
					m_pokeName[i].setText(GameClient.getInstance().getOurPlayer().getMonsters()[i].getName());
					m_pokeIcon[i].setImage(GameClient.getInstance().getOurPlayer().getMonsters()[i].getIcon());
					m_hp[i].setMaximum(GameClient.getInstance().getOurPlayer().getMonsters()[i].getMaxHP());
					m_hp[i].setForeground(Color.green);
					m_hp[i].setValue(GameClient.getInstance().getOurPlayer().getMonsters()[i].getCurHP());
					if (GameClient.getInstance().getOurPlayer().getMonsters()[i].getCurHP() > GameClient.getInstance().getOurPlayer().getMonsters()[i].getMaxHP() / 2) {
						m_hp[i].setForeground(Color.green);
					} else if (GameClient.getInstance().getOurPlayer().getMonsters()[i].getCurHP() < GameClient.getInstance().getOurPlayer().getMonsters()[i].getMaxHP() / 2
							&& GameClient.getInstance().getOurPlayer().getMonsters()[i].getCurHP() > GameClient.getInstance().getOurPlayer().getMonsters()[i].getMaxHP() / 3) {
						m_hp[i].setForeground(Color.orange);
					} else if (GameClient.getInstance().getOurPlayer().getMonsters()[i].getCurHP() < GameClient.getInstance().getOurPlayer().getMonsters()[i].getMaxHP() / 3) {
