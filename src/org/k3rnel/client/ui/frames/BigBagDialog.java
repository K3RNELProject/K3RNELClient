...
import org.k3rnel.client.ui.base.ImageButton;
...
public class BigBagDialog extends Frame {
	...
	private void loadItems()
	{
		...
		for (PlayerItem item : GameClient.getInstance().getOurPlayer().getItems()) {
			...
			// Capture Items
			else if (item.getItem().getCategory().equalsIgnoreCase("Capture")) {
				m_items.get(3).add(item);
			}
			// Moves
			else if (item.getItem().getCategory().equalsIgnoreCase("Move")) {
	...
	public void removeItem(int id, boolean remove) {
		...
		if (remove) {
			 ...
			 */
			int MovepCurCategory = m_curCategory;
			initGUI();
			loadItems();
			m_curCategory = MovepCurCategory;
		}
		m_update = true;
	}
	...
	public void initGUI() {
		...
		try {
			...
			Image[] pokecat = new Image[] {
					new Image(FileLoader.loadFile(respath+"res/ui/bag/Capture Items_normal.png"), "res/ui/bag/Capture Items_normal.png", false),
					new Image(FileLoader.loadFile(respath+"res/ui/bag/Capture Items_hover.png"), "res/ui/bag/Capture Items_hover.png", false),
					new Image(FileLoader.loadFile(respath+"res/ui/bag/Capture Items_pressed.png"), "res/ui/bag/Capture Items_pressed.png", false) };
			Image[] Movescat = new Image[] {
					new Image(FileLoader.loadFile(respath+"res/ui/bag/Moves_normal.png"), "res/ui/bag/Moves_normal.png", false),
					new Image(FileLoader.loadFile(respath+"res/ui/bag/Moves_hover.png"), "res/ui/bag/Moves_hover.png", false),
					new Image(FileLoader.loadFile(respath+"res/ui/bag/Moves_pressed.png"), "res/ui/bag/Moves_pressed.png", false) };
			for (int i = 0; i < m_categoryButtons.length; i++) {
				...
				case 3:
					m_categoryButtons[i] = new ImageButton(pokecat[0],
							pokecat[1], pokecat[2]);
					m_categoryButtons[i].setToolTipText("Capture Items");
					break;
				case 4:
					m_categoryButtons[i] = new ImageButton(Movescat[0],
							Movescat[1], Movescat[2]);
					m_categoryButtons[i].setToolTipText("Moves");
					break;
				}
...
class ItemPopup extends Frame{
	 ...
	 * @param isBattle
	 */
	public ItemPopup(String item, int id, boolean useOnMonster, boolean isBattle){
		final int m_id = id;
		final boolean m_useOnPoke = useOnMonster;
...
class TeamPopup extends Frame{
	...
	public TeamPopup(ItemPopup parent, int itemId, boolean use, boolean isBattle) {
		...
		for (int i = 0; i < GameClient.getInstance().getOurPlayer().getMonsters().length; i++) {
			try{
				final Label tempLabel = new Label(GameClient.getInstance().getOurPlayer().getMonsters()[i].getName());
				final int j = i;
