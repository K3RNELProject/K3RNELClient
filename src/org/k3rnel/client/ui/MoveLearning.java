...
import org.k3rnel.client.ui.base.BattleButtonFactory;
...
public class MoveLearning extends Frame {
	...
	private void replaceMove(int i) {
		...
		if (!GameClient.getInstance().getDisplay().containsChild(m_replace)) {
			if (m_moveButtons.get(i).getText().equals("")) {
				GameClient.getInstance().getOurPlayer().getMonsters()[m_pokeIndex].setMoves(j, m_move);
	...
	public void learnMove(int pokeIndex, String move) {
		...
		GameClient.getInstance().getUi().talkToNPC(GameClient.getInstance().getOurPlayer()
				.getMonsters()[pokeIndex].getName() + " wants to learn " + move);

		m_move = move;

		move1.setText(GameClient.getInstance().getOurPlayer().getMonsters()[m_pokeIndex].getMoves()[0]);
		move2.setText(GameClient.getInstance().getOurPlayer().getMonsters()[m_pokeIndex].getMoves()[1]);
		move3.setText(GameClient.getInstance().getOurPlayer().getMonsters()[m_pokeIndex].getMoves()[2]);
		move4.setText(GameClient.getInstance().getOurPlayer().getMonsters()[m_pokeIndex].getMoves()[3]);

		for (int i = 0; i < 4; i++) {
			if (m_moveButtons.get(i).getText().equals("")) {
				m_pp.get(i).setVisible(false);
			} else {
				m_pp.get(i).setText(GameClient.getInstance().getOurPlayer().getMonsters()[pokeIndex].
						getMoveCurPP()[i] + "/" + GameClient.getInstance().getOurPlayer().getMonsters()
...
class MoveLearnCanvas extends Container {
	...
	public void draw(int pokeIndex) {
		poke = new Label(GameClient.getInstance().getOurPlayer().getMonsters()[pokeIndex].getSprite());
		poke.setSize(80, 80);
