...
import org.k3rnel.client.ui.frames.BattleBag;
...
public class BattleWindow extends Frame {
	...
	public void showMonsterPane(boolean isForced) {
		BattleManager.getInstance().updateMonsterPane();
		attackPane.setVisible(false);
		// bagPane.setVisible(false);
	...
	private void useMove(int i) {
		disableMoves();
		if (BattleManager.getInstance().getCurMonster().getMoveCurPP()[i] != 0) {
			BattleManager.getInstance().getCurMonster().setMoveCurPP(i, 
					BattleManager.getInstance().getCurMonster().getMoveCurPP()[i] - 1);
