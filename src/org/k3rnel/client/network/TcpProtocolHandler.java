package org.k3rnel.client.network;
...
import org.k3rnel.client.backend.MoveLearningManager;
import org.k3rnel.client.backend.Translator;
import org.k3rnel.client.backend.entity.Obstacle;
import org.k3rnel.client.backend.entity.OurPlayer;
import org.k3rnel.client.backend.entity.Player;
import org.k3rnel.client.backend.entity.Player.Direction;
import org.k3rnel.client.backend.time.WeatherService.Weather;
import org.k3rnel.client.ui.frames.SpriteChooserDialog;
	...
	public void messageReceived(IoSession session, Object m) {
		...
		case 'T':
			...
			case 'i':
				...
				GameClient.getInstance().getUi().getTrade().addMonster(Integer.parseInt(String.valueOf(
			...
			break;
		case 's':
			//Party swapping. Received as s0,5. Monsters in party at 0 and 5 were swapped around
			m_game.getOurPlayer().swapMonster(Integer.parseInt(message.substring(1, message.indexOf(','))),
		...
		case 'b':
			...
			case 'P':
				...
				BattleManager.getInstance().setEnemyMonster(Integer.parseInt(data[0]),
			...
			case 'F':
				...
				BattleManager.getInstance().getTimeLine().informFaintedMonster(
						message.substring(2));
				break;
			case 'M':
			...
			case '.':
				...
				break;
			case 'e':
				//A Monster received a status effect -> beTRAINER.POKEMON,EFFECT
				...
				break;
			case 'E':
				//A Monster had a status effect removed -> bEPOKEMON,EFFECT
				BattleManager.getInstance().getTimeLine().informStatusHealed(
						Integer.parseInt(String.valueOf(message.charAt(2))), message.substring(3).split(","));
				break;
			case 's':
				//Switch in Monster requested
		...
		case 'P':
			...
			case 'S':
				...
				m_game.getOurPlayer().updateMonster(Integer.parseInt(String.valueOf(message.charAt(2))), details);
				m_game.getUi().refreshParty();
				break;
			case 'N':
				//A pokemon left the party
				m_game.getOurPlayer().setMonster(Integer.parseInt(String.valueOf(message.charAt(2))), null);
				m_game.getUi().refreshParty();
				break;
			case 'i':
				//Initialise a pokemon
				details = message.substring(3).split(",");
				m_game.getOurPlayer().setMonster(Integer.parseInt(message.substring(2, 3)), details);
				break;
			case 'm':
				/*
			...
			case 'M':
				...
				m_game.getOurPlayer().getMonsters()[Integer.parseInt(String.valueOf(message.charAt(2)))]
			...
			case 'e':
				...
				int exp = m_game.getOurPlayer().getMonsters()[p1].getExp() + Integer.parseInt(message.substring(3));
				m_game.getOurPlayer().getMonsters()[p1].setExp(exp);
				m_game.getUi().update(false);
				break;
			case 'E':
				/*
			...
			case 'l':
				...
				m_game.getOurPlayer().getMonsters()[Integer.parseInt(levelData[0])].setLevel(
						Integer.parseInt(levelData[1]));
				m_game.getUi().update(false);
				break;
			case 'h':
			...
			case 'p':
				...
				m_game.getOurPlayer().getMonsters()[poke].setMoveCurPP(move, Integer.parseInt(String.valueOf(details[0])));
				m_game.getOurPlayer().getMonsters()[poke].setMoveMaxPP(move, Integer.parseInt(String.valueOf(details[1])));
				break;
			}
			break;
		case 'F':
			...
			case 'H':
				...
				for (int i = 0; i < GameClient.getInstance().getOurPlayer().getMonsters().length; i++){
					if (GameClient.getInstance().getOurPlayer().getMonsters()[i] != null){
						GameClient.getInstance().getOurPlayer().getMonsters()[i].setCurHP(
								GameClient.getInstance().getOurPlayer().getMonsters()[i].getMaxHP());
						for (int x = 0; x < 4; x++){
							GameClient.getInstance().getOurPlayer().getMonsters()[i].setMoveCurPP(x, 
									GameClient.getInstance().getOurPlayer().getMonsters()[i].
									getMoveMaxPP()[x]);
						}
					}
				}
		...
		case 'm':
			...
			case 'i':
				...
				for(int i = 0; i < details.length - 1; i++) {
					p = new Player();
					try {
						Obstacle hm = new Obstacle(Obstacle.parseHMObject(details[i]));
						i++;
			...
			case 'a':
				...
				p = new Player();
				try {
					Obstacle hm = new Obstacle(Obstacle.parseHMObject(details[0]));
